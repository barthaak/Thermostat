...
*/
    ...
    public String timer(String theday){

        for (int j = 0; j < data.get(theday).size() - 1; j++) {

            if (data.get(theday).get(j + 1).getState()) {
                thetime = data.get(theday).get(j + 1).getTime();

            } else {
                thetime = data.get(theday).get(j).getTime();
